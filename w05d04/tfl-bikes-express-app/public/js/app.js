...
$(function () {
  ...
  function initMap() {
    var uluru = { lat: 51.515209, lng: -0.072132 };
    ...
    function getBikes() {
      $.get('https://api.tfl.gov.uk/bikepoint').done(function (response) {
        for (var i = 0; i < response.length; i++) {
          ...
          var contentString = '<div id="content">' + '<div id="siteNotice">' + '</div>' + ('<h1 id="firstHeading" class="firstHeading">' + response[i].commonName + '</h1>') + '<div id="bodyContent">' + ('<p><b>Available Bikes: </b> ' + response[i].additionalProperties[6].value + ' </p>') + ('<p><b>Empty Docks: </b> ' + response[i].additionalProperties[7].value + ' </p>') + '</div>' + '</div>';
